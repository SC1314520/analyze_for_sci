...
@WebServlet("/verify")
public class visitor_verification extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			if(name.length()==0|| mail.length()==0|| age.length()==0|| sex.length()==0|| birth.length()==0|| Arrays.toString(doing).length()==0|| id.length()==0) {
				resp.sendError(500, "資料不可空白");
				return;
			}
			else {
				String[] str= (id.split(""));
